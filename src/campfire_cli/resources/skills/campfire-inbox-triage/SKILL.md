...
1. 区分 `_收件箱/用户输入/` 的原始材料与 `_收件箱/待用户确认/` 的 Decision 只读投影，并保留原文；投影的事实必须通过 `campfire decision show` 读取。
...
- `medium` 或 `low` 必须请求确认。
- Decision 必须关联原文、列出候选及一个明确问题，不能代替正式知识文档；`待确认-Decision-*.md` 不允许手工维护。
