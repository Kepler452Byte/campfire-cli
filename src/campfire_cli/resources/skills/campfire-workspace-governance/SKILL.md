...
description: "治理 Campfire Workspace 中的共享上下文；适用于收件箱分流、笔记分类、结构重构、沉淀、MOC、领域声明和治理检查。"
---
...
先运行 `campfire workspace resolve` 获取目标 Workspace；需要指定其他 Workspace 时使用 `campfire --workspace <id>`。然后从返回的 Workspace 根目录读取 `AGENTS.md` 和治理 SPEC。本 Skill 只处理分类、结构重构、MOC、链接、Frontmatter、领域声明和治理检查；对话意图、具体任务、知识写作和周报分别由对应的垂直 Skill 处理。
...
3. 一次性存量治理使用 `campfire workspace restructure inventory --scope <path> --batch <id>` 与 `campfire workspace restructure plan --batch <id>`；工具无法推断的移动或元数据修改写入 YAML/JSON 意图规格，并使用 `campfire workspace restructure plan --batch <id> --spec <file>`。日常新增和变化使用 `campfire maintenance plan`。所有计划默认未审批，有关键歧义时不执行。
4. 只在用户授权范围内整理笔记。删除、合并、领域拆分、顶级领域创建和冲突权威判定必须由用户确认。
5. 写入前先运行不带 `--confirm` 的 `campfire workspace restructure apply` 或 `campfire maintenance apply`；只有计划已明确审批且预检通过时才追加 `--confirm`。
6. 使用 `campfire maintenance sync --dry-run` 预览生成物，再运行 `campfire maintenance sync`。批量治理最后运行 `campfire workspace restructure verify --batch <id>`；日常维护运行 `campfire maintenance run`。
...
- 一次性治理与日常维护使用不同命令和状态；Agent 不得用 Workspace Restructure 入口处理普通增量维护。
- 写入返回 `concurrent-change`、`source-hash-changed` 或 `restructure-config-changed` 时，必须停止并重新审查，不得绕过快照保护。
