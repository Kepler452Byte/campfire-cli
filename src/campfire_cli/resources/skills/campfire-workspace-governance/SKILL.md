...
description: "治理 Vault 中的全局收件箱、正式领域和知识笔记；适用于笔记分类、迁移、沉淀、MOC、相关文档、领域声明和治理检查。"
---
...
先运行 `campfire workspace resolve` 获取目标 Vault；需要指定其他 Vault 时使用 `campfire --workspace <id>`。然后从返回的 Vault 根目录读取 `spec/规范-SPEC-知识库人机协作治理方案.md`。处理 `_收件箱/` 时读取同级全局 Skill `campfire-inbox-triage`；处理 `mynote/`、`mywork/`、任务或周报时加载对应全局 Skill。

## 工作流

1. 确认已安装 `campfire-cli`，运行 `campfire workspace resolve` 和 `campfire maintenance check` 获取当前 Vault 与状态；该命令不改原始笔记，但会刷新用户级治理状态，不得直接读写 `~/.campfire/`。
...
- 整个 Vault 只有根目录一个 `_收件箱/`；领域内部不创建收件箱。
