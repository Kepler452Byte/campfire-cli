...
4. 用户说“记一下”“帮我跟进”“交给 Agent”“老板交代”等，视为允许捕获任务草案，但不等于允许实施。读取[模糊任务 SOP](references/task-intake.md)。
...
涉及 Vault 写入时同时遵守 `campfire-workspace-governance`；创建正式任务时遵守 `mywork-task-governance`；沉淀长期知识时遵守 `mynote-knowledge-governance`；软件开发任务还要读取项目自身 `AGENTS.md` 和 `tasks/SPEC.md`。
...
- 对话中的任务草案可以进入 `draft`；只有用户确认目标、范围和验收后才能进入 `todo` 并分派执行。
