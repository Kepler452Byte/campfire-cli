...
无法唯一分类或定位 Project/Domain 时询问用户，或在 `_待用户确认/` 创建 `human-request`，不同时加载多条参考。
...
单篇文档只改标题时使用 `document rename --path <source> --name <标题>`；它保留原目录，并同步 `name`、规范文件名和引用。跨 Domain 归属移动才使用 `document move`。Domain 合并或逻辑空删除加载 `campfire-workspace-restructure` 并使用 `domain merge/delete`，批量文档迁移或领域拆分才使用持久 restructure 批次。CLI 写入完成后只执行结果中实际返回的 `follow_up`，当前只可能是一个最小 scope 的 `maintenance sync`；没有 follow-up 就结束，不自行追加 dry-run、check 或全 Workspace 扫描。本 Skill 不复制文档 Profile、生命周期、字段枚举或结构重构步骤。

## 待用户确认

对象、资源、事实或目标文档存在关键歧义时，创建 `_待用户确认/待确认-*.md` 的 `human-request`。它是全局系统受管区，不属于任何 Space、Domain 或 Project；只记录待回答问题、已确认事实与候选，不替代正式文档。用户回答后将结论写回正式文档。只有用户明确要求或同意后，才可将该事项或任何其他文档设为 `document_status: archived`；`--confirm` 只确认 CLI 写入，不构成归档授权。
